...
# From the company database, retrieve the number employees 
# who have the lowest salary among all employees. Turn in SQL statement and output.
...
# MySQL Trigger 
CREATE TRIGGER SALARY_VIOLATION # name of trigger 
BEFORE INSERT ON EMPLOYEE  # specify events 
FOR EACH ROW  # condition set with WHEN 
...
SET NEW.SALARY = IF (   
  ...
  NULL,  # if greater, we set the salary of new insertion to NULL 
      NEW.SALARY
);
...
SELECT Dname, COUNT(*) AS No_of_emps
FROM EMPLOYEE JOIN DEPARTMENT ON Dno = Dnumber 
GROUP BY Dname 
HAVING AVG(Salary) > 30000; 


SELECT Dname, COUNT(*) AS No_of_male_emps
FROM EMPLOYEE JOIN DEPARTMENT ON Dno = Dnumber 
...
CREATE UNIQUE INDEX DnoIndex
ON EMPLOYEE (Dno ASC);

DROP INDEX DnoIndex ON EMPLOYEE;
